...
def test_all():
    ...
    with pytest.raises(linkrot.exceptions.PDFInvalidError):
        linkrot.PDFx(os.path.join(curdir, "pdfs/invalid.pdf"))

    pdf = linkrot.PDFx(os.path.join(curdir, "pdfs/valid.pdf"))
...
def test_two_pdfs():
    ...
    linkrot.PDFx(os.path.join(curdir, "pdfs/i14doc1.pdf"))
    pdf_2 = linkrot.PDFx(os.path.join(curdir, "pdfs/i14doc2.pdf"))
