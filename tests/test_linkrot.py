...
def test_linkrot_exceptions(
    download_pdfs,
    test_case: Literal[
        'asd',
        'http://invalid.com/404.pdf',
        './tests/pdfs/invalid.pdf'
    ],
    expected: type[FileNotFoundError] | type[DownloadError] | type[PDFInvalidError]
):
    with pytest.raises(expected):
        linkrot.linkrot(test_case)
...
def test_pdf_with_email_address():
    pdf_with_email_addresses = linkrot(
        "./tests/pdfs/email_test_single_page.pdf"
    )
...
def test_pdf_with_embedded_links():
    pdf_with_embedded_links = linkrot(
        "./tests/pdfs/embedded_link_testcase.pdf"
    )
...
def test_pdf_with_embedded_link_in_image():
    pdf_with_embedded_link_in_image = linkrot(
        "./tests/pdfs/embedded_link_image.pdf"
    )
    ...
    EMBEDDED_LINK_IN_IMAGE = (
        "https://github.com/marshalmiller/linkrot/blob/"
        "6e6fb45239f8d06e89671e2ec68a11629747355d/"
        "branding/Asset%207@4x.png"
    )
